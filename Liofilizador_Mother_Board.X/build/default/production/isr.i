...
void __attribute__((picinterrupt(("low_priority")))) isr(void)
{
    ...
    if(PIR1bits.TMR1IF==1){
       ...
       ClrReset=1;
       if(statusWDT.flag_main_loop_WDT==0) ClrReset=0;

       for(count=1;count<=totalboard;count++)
           {
           if(flag_array_slave_WDT[count]==0) ClrReset=0;
           }
       if(ClrReset==1)
          {
          __asm("CLRWDT");
          statusWDT.flag_main_loop_WDT=0;
          for(count=1;count<15;count++)
              {
              flag_array_slave_WDT[count]=0;
              }
          }
       ...
       if(tempocaptura>0)
          ...
          statusgen.flag_capture_datalog=1;
          }
